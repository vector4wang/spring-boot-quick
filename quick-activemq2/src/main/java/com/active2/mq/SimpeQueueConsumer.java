...
public class SimpeQueueConsumer {
    ...
    @JmsListener(destination = "${jsa.activemq.simple.queue.name_2}", containerFactory = "jmsSimpeQueueListener2")
    public void receiveQueue2(final TextMessage text, Session session)
            throws JMSException {
        doMsg(text, session, "Consumer2收到的报文为:" + text.getText());
    }

    @JmsListener(destination = "${jsa.activemq.simple.queue.name_3}", containerFactory = "jmsSimpeQueueListener3")
    public void receiveQueue3(final TextMessage text, Session session)
            throws JMSException {
        doMsg(text, session, "Consumer3收到的报文为:" + text.getText());
    }

    @JmsListener(destination = "${jsa.activemq.simple.queue.name_4}", containerFactory = "jmsSimpeQueueListener4")
    public void receiveQueue4(final TextMessage text, Session session)
            throws JMSException {
        doMsg(text, session, "Consumer4收到的报文为:" + text.getText());
    }

    @JmsListener(destination = "${jsa.activemq.simple.queue.name_5}", containerFactory = "jmsSimpeQueueListener5")
